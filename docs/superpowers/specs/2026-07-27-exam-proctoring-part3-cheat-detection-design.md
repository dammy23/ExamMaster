...
- New `client/src/lib/cheatDetection.ts`:
  ...
  - `probeSuspiciousExtensions(): Promise<boolean>` — fetch-probes a small starter list of `{ name, url }` entries (chrome-extension:// web-accessible-resource URLs), each wrapped in its own `try/catch` so one probe's rejection doesn't affect the others; returns `true` if any resolves successfully.
